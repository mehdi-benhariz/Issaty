...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_TEACHER: 'teacher/PARTIAL_UPDATE_TEACHER',
...
export default (state: TeacherState = initialState, action): TeacherState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_TEACHER):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_TEACHER):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_TEACHER):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<ITeacher> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_TEACHER,
