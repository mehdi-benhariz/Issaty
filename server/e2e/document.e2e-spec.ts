...
describe('Document Controller', () => {
    ...
    it('/GET all documents ', async () => {
        const getEntities: DocumentDTO[] = (await request(app.getHttpServer()).get('/api/documents').expect(200)).body;

        expect(getEntities).toEqual(entityMock);
    });

    it('/GET documents by id', async () => {
        const getEntity: DocumentDTO = (
    ...
    it('/POST create documents', async () => {
        const createdEntity: DocumentDTO = (
            await request(app.getHttpServer()).post('/api/documents').send(entityMock).expect(201)
        ).body;
    ...
    it('/PUT update documents', async () => {
        const updatedEntity: DocumentDTO = (
            await request(app.getHttpServer()).put('/api/documents').send(entityMock).expect(201)
        ).body;
    ...
    it('/DELETE documents', async () => {
        const deletedEntity: DocumentDTO = (
            await request(app.getHttpServer())
